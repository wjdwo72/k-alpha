...
제3자에게 제공되지 않습니다. 회원 탈퇴 시 모든 개인정보는 즉시 삭제됩니다.
자세한 내용은 아래 [개인정보 수집 및 이용 동의 전문]을 확인하세요.<br><br>
...
운영자는 책임을 지지 않습니다.
    ...
    # 개인정보 수집 및 이용 동의 전문
    st.markdown("""
<div style="background:rgba(10,14,26,0.85);border:1px solid rgba(100,116,139,0.4);border-radius:12px;
  padding:20px;margin-bottom:14px;text-align:left;">
<div style="color:#94a3b8;font-size:14px;font-weight:700;margin-bottom:12px;">🔒 개인정보 수집 및 이용 동의 전문</div>
<div style="color:#8899bb;font-size:11.5px;line-height:1.9;">

<b style="color:#cbd5e1;">1. 수집하는 개인정보의 항목</b><br>
회사는 원활한 서비스 제공을 위해 아래와 같은 최소한의 개인정보를 수집하고 있습니다.<br>
• <b>필수 항목:</b> 아이디(이메일), 비밀번호, 닉네임, 휴대폰 번호<br>
• <b>자동 수집 항목:</b> 서비스 이용 기록, 접속 로그, 쿠키, 접속 IP 정보, 기기 정보<br><br>

<b style="color:#cbd5e1;">2. 개인정보의 수집 및 이용 목적</b><br>
회사는 수집한 개인정보를 다음의 목적을 위해 활용합니다.<br>
• <b>회원 관리:</b> 회원제 서비스 이용에 따른 본인 확인, 개인 식별, 불량 회원의 부정 이용 방지, 가입 의사 확인<br>
• <b>서비스 제공:</b> 서비스 제공에 관한 계약 이행 및 서비스 제공에 따른 콘텐츠 제공<br>
• <b>마케팅 및 광고:</b> 이벤트 등 광고성 정보 전달 (선택 동의 시에 한함)<br><br>

<b style="color:#cbd5e1;">3. 개인정보의 보유 및 이용 기간</b><br>
회사는 원칙적으로 개인정보 수집 및 이용 목적이 달성된 후에는 해당 정보를 지체 없이 파기합니다.<br>
• <b>보유 기간:</b> 회원 탈퇴 시까지 또는 서비스 제공 목적 달성 시까지<br>
• 단, 관계 법령에 따른 보존:<br>
&nbsp;&nbsp;- 계약 또는 청약 철회 등에 관한 기록: <b>5년</b> (전자상거래 등에서의 소비자보호에 관한 법률)<br>
&nbsp;&nbsp;- 소비자의 불만 또는 분쟁 처리에 관한 기록: <b>3년</b> (전자상거래 등에서의 소비자보호에 관한 법률)<br>
&nbsp;&nbsp;- 본인 확인에 관한 기록: <b>6개월</b> (정보통신망 이용촉진 및 정보보호 등에 관한 법률)<br><br>

<b style="color:#cbd5e1;">4. 동의를 거부할 권리 및 불이익</b><br>
이용자는 개인정보 수집 및 이용 동의를 거부할 권리가 있습니다.
다만, 필수 항목에 대한 동의를 거부하실 경우 서비스 가입 및 이용이 제한될 수 있습니다.
...
<span id="clk" style="font-size:20px;font-weight:700;color:#e2e8f0;letter-spacing:2px;font-family:monospace">00:00:00</span>
</div>
<script>
const DAYS=["일","월","화","수","목","금","토"];
function tick(){
  var now=new Date(), kst=new Date(now.getTime()+9*3600000);
...
@st.dialog("📊 차트 보기", width="large")
def _chart_dialog():
    sc    = st.session_state.get("_dlg_stock", {})
    ctype = st.session_state.get("_dlg_type", "candle")
    name  = sc.get("name",""); code = sc.get("code","")
    ...
    if ctype == "candle":
        ...
        chart_url = f"https://ssl.pstatic.net/imgfinance/chart/item/area/day/{code6}.png"
        label = "선차트(당일)"
    st.markdown(f"**{name}** `{code}`")
    st.markdown(f"""
<div style="background:#0d1520;border:1px solid #1e2a3a;border-radius:10px;padding:10px;margin-bottom:10px">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
    <span style="font-size:13px;color:#94a3b8">{"📊" if ctype=="candle" else "📈"} {label} 미리보기</span>
    <a href="{naver_url}" target="_blank" style="font-size:12px;color:#7dd3fc;text-decoration:none">네이버에서 보기 ↗</a>
  </div>
  <img src="{chart_url}" style="width:100%;border-radius:6px;background:#0a0e1a;display:block"
    onerror="this.style.opacity='0.1'" alt="{label}">
</div>""", unsafe_allow_html=True)
    for icon, title, desc, url, color in [
...
<div style="background:#141929;border:1px solid #1e2a3a;border-radius:10px;
  ...
  <span style="font-size:11px;color:{color}">열기 ↗</span>
...
<div class="stock-card" style="margin-bottom:16px;padding:16px 18px;">
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
    <div>
      ...
      <span style="font-size:10px;color:#475569;background:#1e2a3a;padding:2px 6px;border-radius:4px;margin-left:4px">{mkt}</span>
    </div>
  ...
  <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:10px">
    ...
    <span style="font-size:11px;background:#1e2a3a;color:#e2e8f0;padding:3px 8px;border-radius:10px">매 {buy}원</span>
...
<div style="padding:12px 0 8px">
  ...
  <div style="font-size:15px;color:#94a3b8;margin-top:2px">{name} <span style="font-size:12px;color:#475569">{code}</span></div>
    ...
    with col_close:
        ...
        if st.button("✕ 닫기", key="btn_close_chart"):
...
<div style="background:#141929;border:1px solid #1e2a3a;border-radius:10px;
  ...
  <span style="font-size:12px;color:{color};font-weight:600">열기 ↗</span>
</div></a>
""", unsafe_allow_html=True)
