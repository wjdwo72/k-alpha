...
st.markdown("""<style>
  ...
  /* 경고/에러 메시지 작게 */
  div[data-testid="stNotification"]{padding:6px 10px!important;font-size:12px!important}
...
.nb{{padding:18px 0;border-radius:12px;border:1px solid #1a2535;background:#0d1220;color:#e2e8f0;font-family:'Share Tech Mono',monospace;font-size:20px;cursor:pointer;text-align:center;transition:all .15s;-webkit-tap-highlight-color:transparent;user-select:none}}
.nb:active,.nb:focus{{background:#1a2535;transform:scale(.95);outline:none}}
.nb.d{{font-size:15px;color:#64748b}}.nb.e{{visibility:hidden}}
.le{{text-align:center;color:#ff4d6d;font-size:11px;margin-top:14px;min-height:16px;transition:opacity .3s}}
</style>
<div class="lw">
  ...
  <div class="lb">
    ...
    <div class="ld" id="dots">
      <div class="dot" id="d0"></div><div class="dot" id="d1"></div>
      <div class="dot" id="d2"></div><div class="dot" id="d3"></div>
    </div>
    <div class="np">
      {''.join([f'<div class="nb" onclick="pp({i})" ontouchstart="pp({i})">{i}</div>' for i in [1,2,3,4,5,6,7,8,9]])}
      <div class="nb e"></div>
      <div class="nb" onclick="pp(0)" ontouchstart="pp(0)">0</div>
      <div class="nb d" onclick="pd()" ontouchstart="pd()">⌫</div>
    </div>
    <div class="le" id="le">{"❌ 비밀번호가 틀렸습니다" if st.session_state.wrong else ""}</div>
  </div>
</div>
<script>
let p=""; const c="{PASSWORD}"; let submitted=false;
...
function setVal(inp,v){{try{{const s=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,'value').set;s.call(inp,v);['input','change'].forEach(ev=>inp.dispatchEvent(new Event(ev,{{bubbles:true}})));}}catch(e){{inp.value=v;}}}}
function doSubmit(){{
  const inp=document.querySelector('input[type="password"]');
  if(!inp){{setTimeout(doSubmit,200);return;}}
  setVal(inp,p);
  // Enter 키 이벤트
  ['keydown','keypress','keyup'].forEach(ev=>inp.dispatchEvent(new KeyboardEvent(ev,{{key:'Enter',keyCode:13,bubbles:true}})));
  // 버튼 클릭 fallback
  setTimeout(()=>{{
    const btns=document.querySelectorAll('button');
    btns.forEach(b=>{{if(b.textContent.trim()==='확인')b.click();}});
  }},150);
}}
function pp(n){{
  if(submitted||p.length>=4)return;
  event&&event.preventDefault();
  p+=String(n);ud();
  if(p.length===4){{
    setTimeout(()=>{{
      if(p===c){{submitted=true;doSubmit();}}
      else{{
        document.getElementById("le").textContent="❌ 비밀번호가 틀렸습니다";
        document.querySelectorAll(".dot").forEach(d=>{{d.style.background="#ff4d6d";d.style.borderColor="#ff4d6d";d.style.boxShadow="0 0 8px rgba(255,77,109,.5)";}});
        setTimeout(()=>{{p="";submitted=false;ud();document.getElementById("le").textContent="";}},700);
      }}
    }},80);
  }}
}}
function pd(){{if(submitted)return;event&&event.preventDefault();p=p.slice(0,-1);ud();}}
document.addEventListener("keydown",e=>{{if(e.key>="0"&&e.key<="9")pp(parseInt(e.key));else if(e.key==="Backspace")pd();}});
    ...
    with st.form("pin_form", clear_on_submit=True):
        ...
        submitted_btn = st.form_submit_button("확인", use_container_width=True)
        if submitted_btn or (pi and len(pi)==4):
            if pi == PASSWORD:
...
with st.expander(label, expanded=not bool(st.session_state.kis_token)):
    ...
    # ── 간편비번 저장/불러오기 ──
    st.markdown("""<div style='font-family:monospace;font-size:11px;color:#4a5568;
      background:#0a0e1a;border:1px solid #1a2535;border-radius:8px;
    ...
    save_pin = st.text_input("비번(4자리)", type="password", max_chars=4,
                              placeholder="4자리 숫자", key="save_pin_inp",
                              label_visibility="visible")

    col_s, col_l, col_d = st.columns([1,1,0.3])
    with col_s:
        if st.button("💾 저장", use_container_width=True, key="btn_save"):
            pin_v = save_pin.strip()
            if pin_v and len(pin_v)==4 and pin_v.isdigit():
                ...
                if ak_v and sec_v:
                    qp['ck'] = py_save(ak_v,sec_v,acc_v or '',env_v,pin_v)
    ...
    with col_l:
        if st.button("📂 불러오기", use_container_width=True, key="btn_load"):
            ...
            if pin_v and len(pin_v)==4 and pin_v.isdigit():
                if saved_creds:
                    chk = saved_pin_chk
                    if chk and base64.b64decode(chk).decode() != pin_v+':kalpha':
                        st.error("❌ PIN 틀림")
                    else:
                        try:
                            data = py_load(saved_creds, pin_v)
                ...
                st.error("4자리 숫자 입력")
    with col_d:
        if st.button("🗑", use_container_width=True, key="btn_del"):
    ...
    ak  = st.text_input("앱키", type="password",
                         value=st.session_state.kis_ak,
                         placeholder="PSxxxxxxxx...", key="kis_ak_inp")
    sec = st.text_input("시크릿", type="password",
...
if st.session_state.kis_token:
    ...
    with col_info:
        ...
        if prices:
            ...
            st.caption(f"📊 {len(prices)}종목 · {price_ts}")
