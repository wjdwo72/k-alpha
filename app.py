...
st.markdown("""<style>
  ...
  div[data-testid="stExpander"]{background:#0a0e1a;border:1px solid #1a2535!important;border-radius:8px;margin-bottom:8px}
  div[data-testid="stExpander"] summary{color:#00d4ff;font-family:monospace;font-size:13px}
  .stTextInput>div>div>input{background:#0d1220;color:#e2e8f0;border-color:#1a2535;font-family:monospace}
...
# ── KIS API — 상단 접이식 패널 (모바일 친화적) ──
kis_label = (f"🔑 KIS API  ✅ {st.session_state.kis_env} 연결됨"
             if st.session_state.kis_token else "🔑 KIS API 연결 (탭하여 열기)")

with st.expander(kis_label, expanded=not bool(st.session_state.kis_token)):
    ...
    with c1: ak  = st.text_input("앱키 (App Key)", type="password", value=st.session_state.kis_ak, placeholder="PSxxxxxxxx...")
    with c2: sec = st.text_input("시크릿 (App Secret)", type="password", value=st.session_state.kis_sec)
    with c3: acc = st.text_input("계좌번호", value=st.session_state.kis_acc, placeholder="69108332-01")
    ...
    if st.button("🔗 KIS API 연결", use_container_width=True, type="primary"):
        if not ak or not sec or not acc:
            ...
            with st.spinner("KIS 서버에 토큰 발급 요청 중..."):
                try:
                    ...
                    if d.get("access_token"):
                        ...
                        st.success("✅ 연결 성공! 터미널에 실시간 데이터가 로드됩니다.")
                        st.rerun()
                    else:
                        st.error(f"❌ {d.get('msg1','앱키/시크릿 오류 — KIS에서 토큰 발급 거부')}")
                except Exception as e:
                    st.error(f"❌ 연결 실패: {str(e)[:120]}")

    if st.session_state.kis_token:
        st.info(f"✅ {env_label} 연결됨 · 계좌: {acc}")
